...
$query ="Insert into pisos values(null,'$calle',$numero,$piso,'$puerta',$cp,$metros,'$zona',$precio,'$nombreFichero',$user,'no')" or die ("Error en la consulta".mysqli_error($conexion));
...
<body>
  <script>
   ...
   else if (ss1.length < 7 || isNaN(ss1)) {
    alert("Los numeros del SS deben ser todos numeros ")
    document.getElementById("ss1").focus()
   }
   else if (ss2.length >2 || ss2.length == 0 || isNaN(ss2)) {
    alert("El segundo numero debe ser de 1 o 2 digitos")
...
<h3><?php echo "Bienvenido ". $user?></h3>
    <form action="altapiso.php" method="post" enctype="multipart/form-data">
    ...
    <p id="volver"><a href="indexvendedor.php"><button>Volver</button></a></p>
